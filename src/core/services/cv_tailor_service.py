...
class CVTailorService(BaseService):
    ...
    def _create_comparison_work_item(
        ...
        return ComparisonWorkItem(
            id=original_item.id,
            summary=CVTailorService._create_comparison_field(
                original_item.summary,
                revised_suggestion.revised_summary if revised_suggestion else None,
            ),
            highlights=CVTailorService._create_comparison_field(
    ...
    def create_comparison_cv(
        ...
        compared_prof_title = CVTailorService._create_comparison_field(
        ...
        compared_work_experience = CVTailorService._process_comparable_list(
            original_cv.work_experience,
            ai_suggestions.revised_work_experience,
            CVTailorService._create_comparison_work_item,
            "WorkExperience",
        )
        compared_projects = CVTailorService._process_comparable_list(
            original_cv.projects,
            ai_suggestions.revised_projects,
            CVTailorService._create_comparison_project_item,
            "Project",
        )
        compared_awards = CVTailorService._process_comparable_list(
            original_cv.awards,
            ai_suggestions.revised_awards,
            CVTailorService._create_comparison_award_item,
            "Award",
        )
        compared_publications = CVTailorService._process_comparable_list(
            original_cv.publications,
            ai_suggestions.revised_publications,
            CVTailorService._create_comparison_publication_item,
            "Publication",
        )
    ...
    def tailor_cv(self, original_cv: CVBody, job_description: JobDescriptionFields):
        ...
        try:
            ...
            if not llm_data.response:
                ...
                raise ResponseParsingError(
                    "Internal error: LLM response was not found."
                )
